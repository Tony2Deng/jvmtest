...
public class ClassModifier {
    ...
    public byte[] modifyUtf8Constant(String oldStr, String newStr) {
        ...
        for (int i = 0; i < cpc; i++) {
            int tag = ByteUtils.byte2Int(classByte, offset + U1, U2);
            if (tag == CONSTANT_UTF8_INFO_TAG) {
                int len = ByteUtils.byte2Int(classByte, offset + 1, U2);
                offset += (U1 + U2);
                ...
                if (oldStr.equalsIgnoreCase(str)) {
                    ...
                    classByte = ByteUtils.byteRepalce(classByte, offset -U2, U2, strBytes);
                    return classByte;
                } else {
                    offset += len;
                }
